...
def upsertCsvFiles(salesforceEnvironment) {//}, sobjectType, externalId) {
    ...
    for(csvFile in environmentDetails.csvDataToUpsert) {
        ...
        def upsertCommand = 'sfdx force:data:bulk:upsert --json --sobjecttype ' + csvFile.sobjectType + ' --externalid ' + csvFile.externalIdField + ' --csvfile ' + csvFile.filename + ' --targetusername ' + salesforceEnvironment
